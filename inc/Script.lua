...
redis:set(snfor..'lock:beso:'..msg.chat_id_,'on')
return "🙋🏼‍♂️* ¦* أهلا عزيزي "..msg.TheRankCmd.."\n📡* ¦* تم تعطيل الرفع في المجموعه \n✓"
end
function cmds_off(msg)
if not msg.Creator then return "📛* ¦* هذا الامر يخص {المنشئ,المطور} فقط  \n🚶"
end
redis:set(snfor..'lock:beso:'..msg.chat_id_,'off')
...
if not msg.Admin then return "📛* ¦*هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n💥" end
if not MsgText[2] and msg.reply_id then
if redis:get(snfor..'lock:beso:'..msg.chat_id_) == 'off' then
...
if redis:get(snfor..'lock:beso:'..msg.chat_id_) == 'on' then
...
if (MsgText[1] == "رفع المدير"  or MsgText[1] == "رفع مدير" ) and BesoJoinChannel(msg) then
...
if (MsgText[1] == "تنزيل المدير" or MsgText[1] == "تنزيل مدير" ) and BesoJoinChannel(msg) then
if not msg.Creator then return "📛* ¦*هذا الامر يخص {المطور,المنشئ} فقط  \n💥" end
if not MsgText[2] and msg.reply_id then
...
if (MsgText[1] == "رفع منشى اساسي" or MsgText[1] == "رفع منشئ اساسي") and BesoJoinChannel(msg) then
if not msg.SudoUser then return "📛* ¦*هذا الامر يخص {المطور,المطور الاساسي} فقط  \n💥" end
if not MsgText[2] and msg.reply_id then 
GetMsgInfo(msg.chat_id_,msg.reply_id,action_by_reply,{msg=msg,cmd="setbeso"}) 
return false
end
if MsgText[2] and MsgText[2]:match('^%d+$') then 
GetUserID(MsgText[2],action_by_id,{msg=msg,cmd="setbeso"}) 
return false
end
if MsgText[2] and MsgText[2]:match('@[%a%d_]+') then 
GetUserName(MsgText[2],action_by_username,{msg=msg,cmd="setbeso"}) 
return false
end 
end

if (MsgText[1] == "تنزيل منشى اساسي" or MsgText[1] == "تنزيل منشى اساسي") and BesoJoinChannel(msg) then
if not msg.SudoUser then return "📛* ¦*هذا الامر يخص {المطور,المطور الاساسي} فقط  \n💥" end
if not MsgText[2] and msg.reply_id then 
GetMsgInfo(msg.chat_id_,msg.reply_id,action_by_reply,{msg=msg,cmd="rembeso"}) 
return false
end
if MsgText[2] and MsgText[2]:match('^%d+$') then 
GetUserID(MsgText[2],action_by_id,{msg=msg,cmd="rembeso"}) 
return false
end
if MsgText[2] and MsgText[2]:match('@[%a%d_]+') then 
GetUserName(MsgText[2],action_by_username,{msg=msg,cmd="rembeso"}) 
return false
end 
end


if (MsgText[1] == "رفع منشى" or MsgText[1] == "رفع منشئ") and BesoJoinChannel(msg) then
if not msg.Beso then return "📛* ¦*هذا الامر يخص {المنشئ الاساسي,المطور,المطور الاساسي} فقط  \n💥" end
if not MsgText[2] and msg.reply_id then
...
if (MsgText[1] == "تنزيل منشى" or MsgText[1] == "تنزيل منشئ" ) and BesoJoinChannel(msg) then
if not msg.Beso then return "📛* ¦*هذا الامر يخص {المطور,المنشى الاساسي فقط} فقط  \n💥" end
if not MsgText[2] and msg.reply_id then
...
if MsgText[1] == "رفع مطور" and BesoJoinChannel(msg) then
if not msg.SudoBase then return "📛* ¦*هذا الامر يخص {المطور الاساسي 👨‍💻} فقط  \n💥" end
if not MsgText[2] and msg.reply_id then 
...
if MsgText[1] == "تنزيل مطور" and BesoJoinChannel(msg) then
if not MsgText[2] and msg.reply_id then 
...
if not msg.SudoBase then return "📛* ¦*هذا الامر يخص {المطور الاساسي} فقط  \n💥" end
local GetVerison = https.request('https://github.com/SOURCESNFOR/snfor.github.io/GetVersion.txt') or 0
...
if Text == 'time' or Text == 'الوقت' and BesoJoinChannel(msg) then
...
if Text == 'tophoto' or Text == 'صوره' and tonumber(msg.reply_to_message_id_) > 0 and BesoJoinChannel(msg) then
function tophoto(beso,snfor)   
...
local NUM_MSG_SNFOR = (redis:get(snfor..'num_msg_snfor'..msg.chat_id_) or 5)
if tonumber(msgs) > tonumber(NUM_MSG_SNFOR) then 
...
SendMention(msg.chat_id_,datau.id_,msg.id_,"🙍🏻‍♂ ¦العضو » "..USERNAME.."\n🚸 ¦قمـت بتگرآر آگثر مـن "..NUM_MSG_SNFOR.." رسـآلهہ‌‏ , لذآ تم تقييدگ مـن آلمـجمـوعهہ‌‏ ✓\n",12,USERCAR) 
return false
end)
end 
...
if not redis:sismember(snfor..'beso','Start') then
redis:setex(snfor..'2DaySleep',172800,'uop90')
redis:sadd(snfor..'beso','Start')
end
if not redis:get(snfor..'2DaySleep') and redis:sismember(snfor..'beso','Start') and not redis:sismember(snfor..'beso','End') then
redis:setex(snfor..'4DaySleep',345600,'mohammad')
redis:sadd(snfor..'beso','End')
sendMsg(SUDO_ID,0,"👨‍💻 ↫ اهلا عزيزي المطور")
end
if not redis:get(snfor..'4DaySleep') and not redis:sismember(snfor..'beso','End2') then
sendMsg(SUDO_ID,0,"🙋🏻‍♂ ¦ هلآو حبي")
redis:sadd(snfor..'beso','End2')
end
if not redis:get('Bes') then
redis:setex('Bes',86400,true) 
...
local creator = redis:smembers(snfor..':BESO_BOT:'..GroupS)
if #creator ~= 0 then
json_data =  json_data..',"Beso" : {'
