...
snfor = Token:match("(%d+)")
redis:set(snfor..":VERSION",2.9)
redis:set(snfor..":SUDO_ID:",SUDO_USER)
redis:set(snfor..":DataCenter:",'German')
redis:set(snfor..":UserNameBot:",BOT_User)
redis:set(snfor..":NameBot:",BOT_NAME)
redis:hset(snfor..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3snfor_INSTALL","Yes")
info = {}
...
snfor = Token:match("(%d+)")
our_id = tonumber(snfor)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(snfor..":UserNameBot:")
SUDO_ID = tonumber(redis:get(snfor..":SUDO_ID:"))
SUDO_USER = redis:hgetall(snfor..'username:'..SUDO_ID).username
version = redis:get(snfor..":VERSION")
DataCenter = redis:get(snfor..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..snfor..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(snfor..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(snfor..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(snfor..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(snfor..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور 👨🏽‍💻'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(snfor..':BESO_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي👷🏽'
msg.TheRank = 'المنشىء الاساسي👷🏽 '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(snfor..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء 👷🏽'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(snfor..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير 👨🏼‍⚕️' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(snfor..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن 👨🏼‍🎓'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(snfor..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(snfor..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(snfor..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(snfor..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(snfor..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(snfor..':WELCOME_BOT'),[[🙋🏽‍♂╿ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(snfor..':NameBot:')..[[ ⚜
...
if msg.GroupActive and redis:get(snfor..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(snfor..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(snfor..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(snfor..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.snfor then 
if msg.text and ScriptFile.isnfor then
for k, snfor in pairs(ScriptFile.snfor) do
local SearchText = msg.text:match(snfor)
if SearchText then
if not CheckFlood(msg) then
return false 
end
local GetMsg = ScriptFile.isnfor(msg,{msg.text:match(snfor)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",snfor.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dsnfor then
if ScriptFile.dsnfor(msg) == false then
return false
end
...
if Plug.snfor then 
if msg.text and Plug.isnfor then
for k, snfor in pairs(Plug.snfor) do
local SearchText = msg.text:match(snfor)
if SearchText then
if not CheckFlood(msg) then
return false
end
local GetMsg = Plug.isnfor(msg,{msg.text:match(snfor)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",snfor.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source snfor")
end 

end
else
print("The File Script.lua Not Runing in The Source snfor")
...
function tdcli_update_callback(data)
	...
	redis:srem(snfor..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/SOURCESNFOR/SNFOR/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/SOURCESNFOR/SNFOR/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/SOURCESNFOR/SNFOR/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/SOURCESNFOR/SNFOR/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/SOURCESNFOR/SNFOR/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/SOURCESNFOR/SNFOR/master/plugins/games.lua','./plugins/games.lua')
	...
	mmd = redis:get(snfor..":SUDO_ID:")
redis:hset(snfor..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(snfor..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(snfor..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(snfor..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(snfor..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(snfor..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(snfor..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(snfor..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(snfor..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(snfor..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(snfor..'group:add'..data.chat_.id_) then
	redis:set(snfor..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(snfor..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(snfor..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(snfor..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	snfor..'group:name'..k,IDS.Title,
	snfor..'num_msg_snfor'..k,5,
	snfor..'group:add'..k,true,
	snfor..'lock_link'..k,true,
	snfor..'lock_spam'..k,true,
	snfor..'lock_webpage'..k,true,
	snfor..'lock_markdown'..k,true,
	snfor..'lock_flood'..k,true,
	snfor..'lock_bots'..k,true,
	snfor..'mute_forward'..k,true,
	snfor..'mute_contact'..k,true,
	snfor..'mute_document'..k,true,
	snfor..'mute_inline'..k,true,
	snfor..'lock_username'..k,true,
	snfor..'replay'..k,true
	)
	redis:sadd(snfor..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(snfor..'username:'..ID,'username',user)
	redis:sadd(snfor..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(snfor..'username:'..ID,'username',user)
	redis:sadd(snfor..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(snfor..'username:'..ID,'username',user)
	redis:sadd(snfor..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(snfor..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(snfor..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(snfor..'username:'..data.user_.id_).username
	...
	redis:hset(snfor..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
